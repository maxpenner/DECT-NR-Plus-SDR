...
const std::string hw_soapy_t::name = "soapy";

hw_soapy_t::hw_soapy_t(const hw_config_t& hw_config_)
    : hw_t(hw_config_) {
...
void hw_soapy_t::set_samp_rate(const uint32_t samp_rate_in) {
...
void hw_soapy_t::initialize_device() {
    // now create the USRP
    m_usrp = uhd::usrp::multi_usrp::make(hw_config.usrp_args);

    // ##################################################
    sdr = SoapySDR::Device::make(hw_config.soapysdr_args);
    // ##################################################
...
void hw_soapy_t::initialize_buffer_tx_pool(const uint32_t ant_streams_length_samples_max) {
...
void hw_soapy_t::initialize_buffer_rx(const uint32_t ant_streams_length_samples) {
...
void hw_soapy_t::set_command_time(const int64_t set_time) {
    if (set_time >= 0) {
...
double hw_soapy_t::set_freq_tc(const double freq_Hz) {
...
float hw_soapy_t::set_tx_power_ant_0dBFS_tc(const float power_dBm) {
...
float hw_soapy_t::set_rx_power_ant_0dBFS_tc(const float power_dBm, const size_t idx) {
    const auto achievable_power_gain =
...
#ifdef RADIO_HW_IMPLEMENTS_GPIO_TOGGLE
void hw_soapy_t::toggle_gpio_tc() {
...
void hw_soapy_t::pps_wait_for_next() const {
...
void hw_soapy_t::pps_full_sec_at_next(const int64_t full_sec) const {
    m_usrp->set_time_next_pps(uhd::time_spec_t(full_sec));
}

std::vector<std::string> hw_soapy_t::start_threads() {
...
std::vector<std::string> hw_soapy_t::stop_threads() {
...
void* hw_soapy_t::work_tx_async_helper(void* hw_usrp) {
    // pointer to our hw_usrp instance that was passed to the thread executing this function as
    // argument this
    hw_soapy_t* calling_instance = reinterpret_cast<hw_soapy_t*>(hw_usrp);
...
void* hw_soapy_t::work_tx(void* hw_usrp) {
    // pointer to our hw_usrp instance that was passed to the thread executing this function as
    // argument this
    hw_soapy_t* calling_instance = reinterpret_cast<hw_soapy_t*>(hw_usrp);
...
void* hw_soapy_t::work_rx(void* hw_usrp) {
    // pointer to our hw_usrp instance that was passed to the thread executing this function as
    // argument this
    hw_soapy_t* calling_instance = reinterpret_cast<hw_soapy_t*>(hw_usrp);
