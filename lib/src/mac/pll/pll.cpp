...
pll_t::pll_t(const section3::duration_t beacon_period_)
    ...
    dectnrp_assert(
        dist_max_64 * section2::get_reference_time_accuracy(true).accuracy_ppm / int64_t{1000000} <
            beacon_period.get_N_samples<int64_t>() / PLL_PARAM_PPM_WORST_CASE_FACTOR,
        "ill-defined");

    beacon_time_vec.resize(dist_min_64 / dist_min_accept_64, common::adt::UNDEFINED_EARLY_64);

    warp_factor_ema = common::adt::ema_t<float, float>(1.0f, PLL_PARAM_EMA_ALPHA);
};
...
void pll_t::provide_beacon_time(const int64_t beacon_time_64) {
    ...
    warp_factor_ema.update(static_cast<float>(dist_64) /
                           static_cast<float>(equivalent_dist_in_rx_time_base));

    dectnrp_assert(warp_factor_ema.get_val() < 1.0f + PLL_PARAM_PPM_OUT_OF_SYNC / 1.0e6,
