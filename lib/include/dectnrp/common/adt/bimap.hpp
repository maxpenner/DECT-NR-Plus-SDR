...
class bimap_t {
    public:
        ...
        bool is_v_known(const V& v) const { return bimap.right.find(v) != bimap.right.end(); }

        void set_v(const K k, const V& v) {
            dectnrp_assert(is_k_known(k), "k unknown");
            if constexpr (unique) {
                dectnrp_assert(!is_v_known(v), "v already known");
            }
        ...
        std::optional<V> get_v_as_opt(const K k) {
            return is_k_known(k) ? bimap.left.at(k) : std::nullopt;
        }

        uint32_t get_v_cnt() const { return bimap.right.size(); };

        // ##################################################
        // key getter, key has no setter as we consider the key to be immutable

        bool is_k_known(const K k) const { return bimap.left.find(k) != bimap.left.end(); }
        ...
        std::optional<K> get_k_as_opt(const V& v)
            requires(unique)
        {
            return is_v_known(v) ? bimap.right.at(v) : std::nullopt;
        }

        auto get_k_range(const V& v)
            requires(!unique)
        {
            dectnrp_assert(is_v_known(v), "v unknown");
        ...
        uint32_t get_k_cnt() const { return bimap.left.size(); };
