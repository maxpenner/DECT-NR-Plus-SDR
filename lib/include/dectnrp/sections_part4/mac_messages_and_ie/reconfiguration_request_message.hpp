...
class reconfiguration_request_message_t final : public mmie_packing_peeking_t {
    public:
        ...
        enum class radio_resource_change_t : uint32_t {
            ...
            no_change = 0,
            more_resources_requested,
            less_resources_requested,
            resource_allocation_ie_follows,
            upper
        };

        struct flow_t {
                association_request_message_t::flow_id_t id;
                bool is_released;

                friend bool operator<(const flow_t& lhs, const flow_t& rhs) {
        ...
        std::set<flow_t> flows;
        radio_resource_change_t radio_resource_change;
